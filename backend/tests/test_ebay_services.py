...
from app.routers.ebay import _CONDITION_FALLBACK_CHAIN, _publish_with_condition_retry
...
# ── _publish_with_condition_retry tests ──────────────────────────────────────

def _make_condition_error_exc(detail: str = "Errore condizione eBay: 400") -> HTTPException:
    """Build an HTTPException that mimics a 25059 condition-error response."""
    exc = HTTPException(status_code=502, detail=detail)
    exc.is_condition_error = True  # type: ignore[attr-defined]
    return exc


def _make_product(sku: str = "SKU-TEST", product_id: int = 1, descrizione: str = "Desc") -> SimpleNamespace:
    return SimpleNamespace(id=product_id, sku=sku, descrizione=descrizione)


def _make_listing(quantity: int = 1) -> SimpleNamespace:
    return SimpleNamespace(quantity_published=quantity, ebay_item_id=None, last_sync_at=None, ebay_offer_id=None)


def _retry_kwargs(offer_id: str = "OFFER-ORIG") -> dict:
    """Common keyword arguments for _publish_with_condition_retry."""
    return dict(
        token="token",
...
def test_condition_fallback_chain_contains_expected_conditions():
    """_CONDITION_FALLBACK_CHAIN should include at least USED_GOOD and NEW."""
    assert "USED_GOOD" in _CONDITION_FALLBACK_CHAIN
    assert "NEW" in _CONDITION_FALLBACK_CHAIN
    assert _CONDITION_FALLBACK_CHAIN.index("USED_GOOD") < _CONDITION_FALLBACK_CHAIN.index("NEW")


def test_publish_with_condition_retry_succeeds_on_first_attempt(monkeypatch):
    """When publish_offer succeeds immediately, the helper returns without retrying."""
    inventory_calls = []

    monkeypatch.setattr(
        "app.routers.ebay.EbayOfferService.publish_offer",
        lambda token, offer_id: "LISTING-1",
    )
    monkeypatch.setattr(
        "app.routers.ebay.EbayInventoryService.create_or_update_inventory_item",
        lambda *a, **kw: inventory_calls.append(kw.get("ebay_condition")),
    )

    final_offer_id, listing_id = _publish_with_condition_retry(**_retry_kwargs("OFFER-1"))

    assert listing_id == "LISTING-1"
    assert final_offer_id == "OFFER-1"
    # No fallback inventory update should have been triggered
    assert inventory_calls == []


def test_publish_with_condition_retry_updates_inventory_item_before_retry(monkeypatch):
    """
    When publish_offer fails with is_condition_error=True, the retry must call
    create_or_update_inventory_item with the fallback condition BEFORE recreating
    and publishing the offer.
    """
    inventory_updates = []
    call_order: list[str] = []
    ...
    def mock_publish(token, offer_id):
        ...
        call_order.append(f"publish:{offer_id}")
        return "LISTING-OK"

    def mock_inventory_update(token, sku, product, listing, marketplace_id, ebay_condition=None, **kwargs):
        inventory_updates.append(ebay_condition)
        call_order.append(f"inventory:{ebay_condition}")
    ...
    monkeypatch.setattr(
        ...
        lambda *a, **kw: "OFFER-NEW",
    )

    final_offer_id, listing_id = _publish_with_condition_retry(**_retry_kwargs("OFFER-ORIG"))

    assert listing_id == "LISTING-OK"
    assert final_offer_id == "OFFER-NEW"
    # Inventory must be updated with fallback condition
    assert "USED_GOOD" in inventory_updates
    # Inventory update must happen BEFORE the successful publish
    inv_idx = call_order.index("inventory:USED_GOOD")
    pub_idx = call_order.index("publish:OFFER-NEW")
    assert inv_idx < pub_idx, "inventory_item must be updated before publish"


def test_publish_with_condition_retry_walks_full_chain_on_repeated_failures(monkeypatch):
    """
    When every condition in _CONDITION_FALLBACK_CHAIN fails with errorId 25059,
    the helper tries all of them (in order) and raises the last exception.
    """
    inventory_updates = []
    publish_calls = {"count": 0}
    ...
    def mock_publish(token, offer_id):
        publish_calls["count"] += 1
        raise _make_condition_error_exc(f"condition error #{publish_calls['count']}")
    ...
    monkeypatch.setattr(
        "app.routers.ebay.EbayOfferService.create_offer",
        lambda *a, **kw: "OFFER-RETRY",
    )

    with pytest.raises(HTTPException) as exc_info:
        _publish_with_condition_retry(**_retry_kwargs("OFFER-ORIG"))

    # Every condition in the chain must have been attempted
    for condition in _CONDITION_FALLBACK_CHAIN:
        assert condition in inventory_updates, f"Expected retry with {condition}"
    # Should have called publish once (initial) + once per chain entry
    assert publish_calls["count"] == 1 + len(_CONDITION_FALLBACK_CHAIN)
    # Last raised exception must still be a condition error (the final one in the chain)
    assert getattr(exc_info.value, "is_condition_error", False) is True


def test_publish_with_condition_retry_stops_at_first_success_in_chain(monkeypatch):
    """
    When the second condition in the chain succeeds, the remaining conditions
    must NOT be tried.
    """
    inventory_updates = []
    publish_calls = {"count": 0}
    ...
    def mock_publish(token, offer_id):
        ...
        if publish_calls["count"] <= 2:
            # First attempt (initial) + first fallback → both fail
            raise _make_condition_error_exc()
        return "LISTING-SECOND-FALLBACK"
    ...
    monkeypatch.setattr(
        "app.routers.ebay.EbayOfferService.create_offer",
        lambda *a, **kw: "OFFER-FB",
    )

    _, listing_id = _publish_with_condition_retry(**_retry_kwargs("OFFER-ORIG"))

    assert listing_id == "LISTING-SECOND-FALLBACK"
    # Only the first two conditions should have been tried (1st fails, 2nd succeeds)
    assert len(inventory_updates) == 2
    assert inventory_updates[0] == _CONDITION_FALLBACK_CHAIN[0]
    assert inventory_updates[1] == _CONDITION_FALLBACK_CHAIN[1]
    # Third condition should NOT have been used
    if len(_CONDITION_FALLBACK_CHAIN) > 2:
        assert _CONDITION_FALLBACK_CHAIN[2] not in inventory_updates


def test_publish_with_condition_retry_propagates_non_condition_error(monkeypatch):
    """
    A non-condition HTTPException raised during publish must be re-raised immediately
    ...
    def mock_publish(token, offer_id):
        ...
        if publish_calls["count"] == 1:
            raise _make_condition_error_exc()
        # Second call: a different (non-condition) error
        raise HTTPException(status_code=502, detail="Errore generico eBay")
