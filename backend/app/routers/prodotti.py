...
def get_prodotti(skip: int = 0, limit: int = 100, db: Session = Depends(get_db)):
    return crud.get_prodotti(db, skip=skip, limit=limit)


@router.get("/sotto-scorta", response_model=List[ProdottoResponse])
def get_prodotti_sotto_scorta(db: Session = Depends(get_db)):
...
def update_prodotto(prodotto_id: int, prodotto: ProdottoUpdate, db: Session = Depends(get_db)):
