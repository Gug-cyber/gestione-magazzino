...
def get_movimento(movimento_id: int, db: Session = Depends(get_db), current_user=Depends(get_current_active_user)):
...
def get_movimenti_by_prodotto(prodotto_id: int, db: Session = Depends(get_db), current_user=Depends(get_current_active_user)):
    return crud.get_movimenti_by_prodotto(db, prodotto_id)


@router.post("/", response_model=MovimentoResponse, status_code=201)
def create_movimento(movimento: MovimentoCreate, db: Session = Depends(get_db), current_user=Depends(get_current_active_user)):
    return crud.create_movimento(db, movimento)


@router.put("/{movimento_id}", response_model=MovimentoResponse)
def update_movimento(movimento_id: int, movimento: MovimentoUpdate, db: Session = Depends(get_db), current_user=Depends(get_current_active_user)):
...
def delete_movimento(movimento_id: int, db: Session = Depends(get_db), current_user=Depends(get_current_active_user)):
